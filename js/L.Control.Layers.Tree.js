  ...
  L.Control.Layers.Tree = L.Control.Layers.extend({
    ...
    onAdd: function (map) {
      ...
      if (this.options.namedToggle) {
        var toggle = this._container.getElementsByClassName(
    ...
    expandSelected: function (overlay) {
      ...
      var inputs =
        this._layerControlInputs || container.getElementsByTagName("input");
      for (var i = 0; i < inputs.length; i++) {
        // Loop over every (valid) input.
        var input = inputs[i];
        if (
          this._getLayer &&
          !!this._getLayer(input.layerId).overlay != !!overlay
        )
          continue;
        if (input.checked) {
    ...
    _localExpand: function () {
      if (
        this._map &&
        L.DomUtil.hasClass(this._container, "leaflet-control-layers-expanded")
      ) {
    ...
    _iterateTreeLayout: function (
      ...
      var closed = creator(
        ...
        this.cls.closed,
        sel,
        this.options.closedSymbol
      );
      var opened = creator(
        "span",
        this.cls.opened,
        sel,
        this.options.openedSymbol
      );
      var space = creator(
        "span",
        this.cls.space,
        null,
        this.options.spaceSymbol
      );
      ...
      if (tree.layer) {
        if (
          this.options.labelIsSelector === "both" || // if option is set to both
          ...
          // if a base layer and option is set to base
          labelType = "label";
        } else {
          // if option is set to something else
          labelType = "span";
        }
      } else {
        labelType = "span";
